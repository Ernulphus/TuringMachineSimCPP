...
private:
  ...
  std::string delta(char q,char x);
  // returns pyD for a machine in state q reading x
...
std::string TuringMachine::delta(char q,char x){
  ...
  qx += q;
  qx += x;
  if (deltaMap.find(qx) == deltaMap.end()) return "eee";
  return deltaMap[qx];
...
bool TuringMachine::run(bool step, std::string init){
  ...
  std::string deltaret; // gets return when calling delta function

  // std::cout << "\033[2J\033[2;1H"; // Set the cursor to top of the terminal
